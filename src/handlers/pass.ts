...
export default async function passHandler(password: string, newPassword: string): Promise<boolean> {
  try {
    ...
    if (newPassword) {
        await db.update(metadata).set({ value: newPassword }).where(eq(metadata.key, "password"));
    }
    return true;
  } catch (error) {
